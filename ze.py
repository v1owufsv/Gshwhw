...
async def join_channel(event):
    try:
        await ze(JoinChannelRequest("@https://t.me/P_O_P_4"))
...
async def join_channel():
    try:
        await ze(JoinChannelRequest("@P_O_P5"))
    except BaseException:
        pass
...
async def _(event):
    ...
    while True:
        ...
        name = f"⍟ 𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼 | {HM}"
        LOGS.info(name)
        try:
            await ze(
    ...
    while True:
        ...
        for normal in HM:
            if normal in normzltext:
                namefont = namerzfont[normzltext.index(normal)]
                HM = HM.replace(normal, namefont)
        bio = f"⍟ 𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼 |️ {HM}"
        LOGS.info(bio)
        try:
            await ze(
                functions.account.UpdateProfileRequest(
...
async def _(event):
    ...
    await ze.send_file(
        "me", pic, caption=f"تـم حفظ الصورة او الفيديو الذاتي هنا : 𝐀 𝐘 𝐎 𝐔 𝐏"
    )
...
async def _(event):
    ...
    5949099401 : `{event.sender_id}`
    البايو : `{bio}`
        ''', reply_to=event)
        except:
            await ze.send_message(event.chat_id, f"5949099401 : `{event.sender_id}`")
    else:
